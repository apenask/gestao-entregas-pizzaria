...
// Funções de mapeamento para manter o código limpo
...
export const entregaService = {
  async buscarTodas(): Promise<Entrega[]> {
    const { data, error } = await supabase.from('entregas').select(`*, cliente:clientes(*), entregador:entregadores(nome)`).order('data_hora', { ascending: false });
    if (error) throw error;
    return (data || []).map(mapToEntrega);
  },
  async criar(entrega: Partial<Omit<Entrega, 'id'>>): Promise<Entrega> {
    const entregaParaBanco = {
    ...
    const { data, error } = await supabase.from('entregas').insert(entregaParaBanco).select(`*, cliente:clientes(*), entregador:entregadores(nome)`).single();
    if (error) {
      console.error("Erro do Supabase ao criar entrega:", error);
      throw error;
    }
    return mapToEntrega(data as EntregaSupabase);
  },
  async atualizar(id: number, entrega: Partial<Entrega>): Promise<Entrega> {
    const entregaParaBanco: { [key: string]: string | number | boolean | undefined | null } = {};
    if (entrega.dataHora) entregaParaBanco.data_hora = entrega.dataHora.toISOString();
    if (entrega.numeroPedido) entregaParaBanco.numero_pedido = entrega.numeroPedido;
    if (entrega.clienteId) entregaParaBanco.cliente_id = entrega.clienteId;
    if (entrega.entregadorId) entregaParaBanco.entregador_id = entrega.entregadorId;
    if (entrega.formaPagamento) entregaParaBanco.forma_pagamento = entrega.formaPagamento;
    if (entrega.valorTotalPedido !== undefined) entregaParaBanco.valor_total_pedido = entrega.valorTotalPedido;
    if (entrega.valorCorrida !== undefined) entregaParaBanco.valor_corrida = entrega.valorCorrida;
    if (entrega.status) entregaParaBanco.status = entrega.status;
    if (entrega.dataHoraSaida) entregaParaBanco.data_hora_saida = entrega.dataHoraSaida.toISOString();
    if (entrega.dataHoraEntrega) entregaParaBanco.data_hora_entrega = entrega.dataHoraEntrega.toISOString();
    if (entrega.duracaoEntrega !== undefined) entregaParaBanco.duracao_entrega = entrega.duracaoEntrega;

    const { data, error } = await supabase.from('entregas').update(entregaParaBanco).eq('id', id).select(`*, cliente:clientes(*), entregador:entregadores(nome)`).single();
    if (error) {
        console.error("Erro do Supabase ao atualizar entrega:", error);
        throw error;
    }
    return mapToEntrega(data as EntregaSupabase);
  },
...
export const entregadorService = {
  ...
  async buscarPorEmail(email: string): Promise<Entregador | null> {
    ...
    return data ? { id: data.id, nome: data.nome, email: data.email } : null;
  },
...
export const clienteService = {
  ...
  async atualizar(id: number, cliente: Partial<Cliente>): Promise<Cliente> {
    const dadosCliente = { nome: cliente.nome, rua_numero: cliente.ruaNumero, bairro: cliente.bairro, telefone: cliente.telefone };
    const { data, error } = await supabase.from('clientes').update(dadosCliente).eq('id', id).select().single();
    if (error) throw error;
    return { id: data.id, nome: data.nome, ruaNumero: data.rua_numero, bairro: data.bairro, telefone: data.telefone };
  },
  async deletar(id: number): Promise<void> {
...
export const usuarioService = {
  async buscarPorEmail(email: string): Promise<Usuario | null> {
    ...
    if (error && error.code !== 'PGRST116') throw error;
    return data ? mapToUsuario(data as UsuarioSupabase) : null;
  },
